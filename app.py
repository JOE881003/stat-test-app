...
if test_type == "Z 檢定":
    ...
    fig, ax = plt.subplots()
    ax.plot(x, y, label="Z-test distribution")
    ax.axvline(z, color="red", linestyle="--", label=f"Z={z:.2f}")
    ax.axvline(critical, color="blue", linestyle="--", label=f"Positive critical = {critical:.2f}")
    ax.axvline(-critical, color="blue", linestyle="--", label=f"Negative critical = {-critical:.2f}")
    ax.set_title("Z-test distribution and critical values")
    ax.legend()
    st.pyplot(fig)
...
# ========== t 檢定 ==========
    ...
    fig, ax = plt.subplots()
    ax.plot(x, y, label="t-test distribution")
    ax.axvline(t, color="red", linestyle="--", label=f"t = {t:.2f}")
    ax.axvline(critical, color="blue", linestyle="--", label=f"Positive critical = {critical:.2f}")
    ax.axvline(-critical, color="blue", linestyle="--", label=f"Negative critical = {-critical:.2f}")
    ax.set_title("t-test distribution and critical values")
    ax.legend()
    st.pyplot(fig)
